...
def read_skeleton_file(filename):
    ...
    frame_count = int(skfile.readline())  # no of the recorded frames
    frameinfo = [Frame() for _ in range(frame_count)]  # [Frame()] * frame_count, uhh not the same, careful with ptrs
...
def worker(input, output):
    ...
    for found_file in iter(input.get, 'STOP'):
        ...
        frames, frame_count = read_skeleton_file(found_file)

        skeletons = {}
        for f, frame in enumerate(frames):
            for body in frame.bodies:
                if body.bodyID not in skeletons:
                    skeletons[body.bodyID] = np.zeros([25, 4, len(frames)])
                for j, joint in enumerate(body.joints):
                    ...
                    skeletons[body.bodyID][j, 3, f] = joint.trackingState

                    # skeletons[body.bodyID][j, 4, f] = joint.orientationW
                    # skeletons[body.bodyID][j, 5, f] = joint.orientationX
                    # skeletons[body.bodyID][j, 6, f] = joint.orientationY
                    # skeletons[body.bodyID][j, 7, f] = joint.orientationZ
                    #
                    # skeletons[body.bodyID][j, 8, f] = joint.colorX
                    # skeletons[body.bodyID][j, 9, f] = joint.colorY
        ...
        pose_array = np.zeros([50, 4, len(frames)], dtype='float32')
        if len(poses) == 1:
            pose_array[0:25, :, :] = poses[0][0]
        elif len(poses) > 1:
            pose_array[0:25, :, :] = poses[0][0]
            pose_array[25:50, :, :] = poses[1][0]
        else:
            print("%s file will be skipped... skeletons %d" % (found_file, len(skeletons)))

        output.put((subject, action, pose_array, frame_count, camera, setup, replication))
...
if __name__ == '__main__':
    ...
    train_cameras = {2, 3}
    h5file_v1 = h5.File(data_set+"v1.h5", "w")
    h5file_v2 = h5.File(data_set+"v2.h5", "w")


    def _write_data(h5file, data_split, pose_array, meta_data, seq_num):
    ...
    def _create_data_set(h5file, data_path, data_array, dtype='int32'):
        h5file.create_dataset(
            data_path, np.shape(data_array),
            dtype=dtype, data=data_array
        )
    ...
    for seq_num in t:
        subject, action, pose_array, frame_count, camera, setup, replication = done_queue.get()
        ...
        if pose_array is not None:
            ...
            data_split_v1 = 'Train' if subject in train_subjects else 'Validate'

            # v2 split (cross view protocol)
            data_split_v2 = 'Train' if camera in train_cameras else 'Validate'
            ...
            _write_data(h5file_v1, data_split_v1, pose_array, meta_data, seq_num)
            _write_data(h5file_v2, data_split_v2, pose_array, meta_data, seq_num)
