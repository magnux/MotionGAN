...
class CuDNNGRUMHDPA(_CuDNNRNN):
    ...
    def __init__(self, units,
                 ...
                 **kwargs):
        self.units = units
        super(CuDNNGRUMHDPA, self).__init__(
            return_sequences=return_sequences,
            return_state=return_state,
            stateful=stateful,
            **kwargs)
    ...
    def build(self, input_shape):
        super(CuDNNGRUMHDPA, self).build(input_shape)
        if isinstance(input_shape, list):
            input_shape = input_shape[0]
        input_dim = input_shape[-1]
        ...
        self.recurrent_kernel = self.add_weight(
            ...
            regularizer=self.recurrent_regularizer,
            constraint=self.recurrent_constraint)
        self.qkv_kernel = self.add_weight(shape=(input_dim, self.units * 3),
                                          name='qkv_kernel',
                                          initializer=self.kernel_initializer,
                                          regularizer=self.kernel_regularizer,
                                          constraint=self.kernel_constraint)
    ...
    def get_config(self):
        config = {
            ...
            'recurrent_constraint': constraints.serialize(self.recurrent_constraint),
            'bias_constraint': constraints.serialize(self.bias_constraint)}
        base_config = super(CuDNNGRUMHDPA, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))