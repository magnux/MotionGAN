        ...
        while config.epoch < config.num_epochs:
            ...
            for batch in t:
                ...
                for disc_batch in range(disc_batches):
                    ...
                    mask_batch = mask_batch * gen_mask(np.random.randint(len(MASK_MODES)), keep_prob, config.batch_size,  # np.random.randint(len(MASK_MODES))
            ...
            mask_batch = poses_batch[..., 3, np.newaxis]
            mask_mode = 1  # np.random.randint(len(MASK_MODES))
            keep_prob = 0.5
